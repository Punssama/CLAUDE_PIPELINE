...
When a run starts, Claude sends you a link like `http://127.0.0.1:3120/#run=...`. You get a link earlier too: a plugin hook shows it **the moment `SPEC.md` and `ROADMAP.md` are both written** (by `/discover`, `/setup-pipeline`, `/toolkit` or by hand), once per session, and a dashboard that is already open jumps to that project by itself. So you can **read `SPEC.md` and `ROADMAP.md` rendered as Markdown before anything is built** (`#project=<id>`; the **Documents** list in the sidebar; the page re-reads the files every couple of seconds, so edits show up while you read). One local page shows **every pipeline run on the machine**, across all your projects:
...
**Stack scan.** `/toolkit` also looks at your project (manifest files, `package.json` dependencies, or `SPEC.md` for a project with no code yet) and suggests plugins that fit the stack. About 30 are in the catalog, each verified to install and measured for its token cost: services and platforms (Vercel, Supabase and Postgres, Stripe, Cloudflare, Clerk, Better Auth, AWS deploy / serverless / Amplify, Azure, Neon, Sanity, Firebase, Terraform, Playwright, Laravel) from their own vendors' marketplaces or the official one, `frontend-design` for web UIs, and the official language-server plugins for TypeScript, Python, Go, Rust, Java, Kotlin, C#, PHP, Ruby, Swift, C/C++ and Lua. **You do not have to ask**: when a Claude Code session starts in a project, a hook says once which of them fit it and are not installed yet (and the notice when `SPEC.md` and `ROADMAP.md` are written adds the ones the SPEC's stack calls for). Suggest-only, with the same one-confirmation install; you can run the scan yourself with `node skills/toolkit/toolkit.mjs --detect <folder>`. The idea (scan the stack, suggest matching skills) is inspired by [autoskills](https://github.com/midudev/autoskills) by midudev; no code or registry from it is used here, since it is CC BY-NC 4.0 and this plugin is MIT.

**Project-only plugins.** When `/toolkit` installs from the project scan, service and platform plugins (Stripe, Supabase, Cloudflare ...) go in for **that project only** (Claude Code's `local` scope: `.claude/settings.local.json`, ignored by git, so the pipeline's clean-working-tree check is not disturbed) and add no tokens to your other projects. Language servers, `frontend-design` and the companion plugins are installed for all projects.

### Clean up after a project: `/clean`

Gave up on a project? `/clean` (or `/clean <folder>`) removes what was only for it and keeps what is shared:

| It removes | It keeps |
|---|---|
| Plugins installed for that project alone (`local` / `project` scope) | Language servers, `frontend-design` and general plugins (ponytail, agent-skills, superpowers ...): shared by nature |
| Service plugins installed for all projects that this project needs and **no other project I have seen** does | Any plugin another project still uses |

"Projects I have seen" means the folders where a Claude session started, plus dashboard registrations and pipeline runs, so the answer is only as good as that memory; anything uncertain is kept. You see the plan first (`toolkit.mjs --clean-plan <folder>`), confirm once, and only ids the plan lists can be uninstalled. Then it offers to mark the project abandoned: it stops counting as a plugin user, leaves the dashboard's Documents list and stops plugin hints (starting a session in it again undoes that). Project files and git history are never touched, and `/toolkit` reinstalls anything with one confirmation.

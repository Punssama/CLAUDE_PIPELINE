...
Once the three files are written, the plugin's hook has already shown the dashboard link by itself (a notice "SPEC.md and ROADMAP.md are ready ... <link>"): keep that link for the summary. If you did not see it, run `node "${CLAUDE_SKILL_DIR}/../setup-pipeline/pipeline.mjs" --dashboard "<project root>"` and use the link it prints (skip this if it says it could not start). Then run `node "${CLAUDE_SKILL_DIR}/../toolkit/toolkit.mjs" --detect "<project root>"`: it reads the SPEC's stack and lists catalog plugins that fit it. Keep only those that are not installed and have no `unmet` (Economy: at most one).
